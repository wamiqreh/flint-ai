...
## Three GPT agents, one pipeline, one call
...
```
🔥 Flint — running workflow 'demo-a3f2c1' (3 nodes)
...
Each agent automatically receives the previous agent's output. No async, no workers, no client setup — just `.run()`.
